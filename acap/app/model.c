...
bool model_run_inference(model_provider_t* provider, VdoBuffer* vdo_buf) {
    ...
    if (provider->use_preprocessing) {
        ...
        syslog(LOG_DEBUG, "Ran pre-processing for %u ms", elapsed_ms(&start_ts, &end_ts));
        nbr_power_retries = 0;
        if (!provider->inf_req) {
        ...
        syslog(LOG_DEBUG, "Ran pre-processing for 0 ms");
    }
    ...
    gettimeofday(&end_ts, NULL);
    syslog(LOG_DEBUG, "Ran inference for %u ms", elapsed_ms(&start_ts, &end_ts));
